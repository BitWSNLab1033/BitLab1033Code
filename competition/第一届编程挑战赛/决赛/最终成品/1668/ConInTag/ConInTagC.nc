...
implementation{
  ...
  event message_t*  AMReceiveTag.receive(message_t* msg,void* p,uint8_t len){
    ...
    if(len == sizeof(tag_msg_t)){
      ...
      tag_msg_t* tag_msg=(tag_msg_t*)call AMSendTag.getPayload(&tag_pkt,sizeof(tag_msg_t));
   ...
   //send to the tagdata to node 1 
    if(TOS_NODE_ID == 2){
    SendE++;
      if(call AMSendTag.send(1,&tag_pkt,sizeof(tag_msg_t))!=SUCCESS){
        call Leds.led0Toggle();
      }else{
